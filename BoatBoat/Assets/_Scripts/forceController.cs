...
public class forceController : MonoBehaviour {
	...
	void OnTriggerStay(Collider collision) {
		if (collision.gameObject.tag == "Whirlpool") {
			...
			this.rigidbody.AddForce(boatToWhirl.normalized * Mathf.Sin((whirlRadius - boatToWhirl.magnitude)/whirlRadius * Mathf.PI/2) * forceFactor * 2);
			this.rigidbody.AddTorque(this.transform.up * rotFactor/(whirlRadius - boatToWhirl.magnitude) * 20);
